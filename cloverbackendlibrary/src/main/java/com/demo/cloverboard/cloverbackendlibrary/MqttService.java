...
//        rebroadcastReceivedMessages();
    ...
    private void broadcastServiceStatus(String statusDescription) {
        ...
        broadcastIntent.setAction(Util.MQTT_STATUS_INTENT);
        broadcastIntent.putExtra(Util.MQTT_STATUS_MSG, statusDescription);
    ...
    private void broadcastReceivedMessage(String topic, String message) {
        ...
        if ((email_id + "/getUser").equals(topic)) {

            broadcastIntent.setAction("getUserData");
            broadcastIntent.putExtra("getUser", message);
            sendBroadcast(broadcastIntent);
        } else if ((email_id + "/getModes").equals(topic)) {
            broadcastIntent.setAction("getUserData");
            broadcastIntent.putExtra("getModes", message);
            sendBroadcast(broadcastIntent);
        } else if ((email_id + "/getUserPlacesNames").equals(topic)) {
            broadcastIntent.setAction("getUserData");
            broadcastIntent.putExtra("getUserPlacesNames", message);
            sendBroadcast(broadcastIntent);
        } else if ((email_id +"/addNewDevice").equals(topic)) {//TODO: change email id with the one stored in
            // SharedPreferences
            broadcastIntent.setAction("addNewDevice");
            broadcastIntent.putExtra("newDeviceInfo", message);

            sendBroadcast(broadcastIntent);
        } else if((email_id + "/addedPlace").equals(topic)){
            broadcastIntent.setAction("addedPlace");
            broadcastIntent.putExtra("Status", message);
        } else if((email_id + "/updateAppliance").equals(topic)){
//            broadcastIntent.setAction("updateAppliance");
//            broadcastIntent.putExtra("status", message);
...
//
//            }
//            DatabaseHandler.getInstance(context).updateAppliance(applianceStatus.getString("app") +
//                    (applianceStatus.getInt("s") +1), messageForUpdate);
//
...
//        }
    ...
    public void publishArrived(String topic, byte[] payloadbytes, int qos,
        ...
        //
        // I'm assuming that all messages I recefive are being sent as strings
    ...
    private boolean connectToBroker() {
        try {
            // try to connect
//            mqttClient
//                    .connect((String) SharedPreferenceManager.getInstance(getApplicationContext()).getPreferenceValue(Util.EMAIL_ID, SharedPreferenceManager.STRING), cleanStart, keepAliveSeconds);
            mqttClient
                    .connect("email_id1", cleanStart, keepAliveSeconds);
    ...
    public class PingSender extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                // reconnect
                if (connectToBroker()) {
                    subscribeToTopic((String) SharedPreferenceManager.getInstance(getApplicationContext()).getPreferenceValue(Util.EMAIL_ID, SharedPreferenceManager.STRING) + "/+");
                }
            }
