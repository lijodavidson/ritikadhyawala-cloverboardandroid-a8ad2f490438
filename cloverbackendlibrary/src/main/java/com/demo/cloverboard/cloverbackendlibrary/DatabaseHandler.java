...
import com.demo.cloverboard.cloverboardlibrary.data.IrRemote;
...
//		Cursor cursor = db.query(APPLIANCES, new String[] { NAME, DEVICE_ID, IS_AUTO, IS_SLIDER, STATUS, APPLIANCE_ID, IS_ON},   DEVICE_ID + " = ?", new String[]{device_id} , null, null, null, null);
	...
	public void updateAppliance(String applianceID, JSONObject value){
		...
		if(applianceID != null  && !applianceID.isEmpty() && value != null){
			...
			Cursor cursor = db.query(APPLIANCES, new String[] {APPLIANCE_ID },  APPLIANCE_ID + " = ?", new String[]{applianceID}, null, null, null, null);
	...
	public void addMode(Theme theme){

		SQLiteDatabase db = this.getReadableDatabase();

		ContentValues cVal = new ContentValues();

		cVal.put(ID, theme.getThemeId());
		cVal.put(NAME, theme.getThemeName());
		cVal.put(PLACE_ID, theme.getPlace_name());
	...
	public List<Theme> getModesOfPlace(String placeID){
		...
		if(cursor.moveToFirst()){
			do{
				themes.add(new Theme(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
			}while(cursor.moveToNext());
		}
		return themes;
	}

	public void deleteMode(Theme theme){

		SQLiteDatabase db = this.getWritableDatabase();
		db.delete(MODES, ID + " =?", new String[]{theme.getThemeId()});

		db.close();
	}

	public void renameMode(String newName, Theme theme){

		SQLiteDatabase db = this.getWritableDatabase();
		if(theme != null && newName != null && !newName.isEmpty()){

			ContentValues cVal = new ContentValues();
			cVal.put(NAME, newName);
			cVal.put(PLACE_NAME, theme.getPlace_name());

			db.update(MODES, cVal, ID + " =?", new String[]{theme.getThemeId()});
		}
	}
	...
	public void updatePlaceName(String placeName, String placeID){
		if(placeName != null && placeID != null){
			...
			addMode(new Theme("General", placeID, "general"));
			addMode(new Theme("Away", placeID, "away"));
	...
	public IrRemote getRemote(String remoteName){
		if(remoteName != null && !remoteName.isEmpty()){
			...
			return new IrRemote(remoteName, cursor.getInt(1), remoteButtons);
		}
		return null;
	}
